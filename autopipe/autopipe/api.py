...
def build_graph_with_nparams_and_grad_reqs(model, model_args, model_kwargs, max_depth, basic_blocks, save_memory_mode,
    ...
    print("WHAT IS GRAPH: "+str(type(graph))+" "+str(graph))
    return graph
...
def get_profiles(graph: Graph, model: nn.Module,
    ...
    if use_graph_profiler:
        ...
        for node in profiler.forward_times:
            print("node name:"+str(node)+" forward_time:"+str(profiler.forward_times[node])+", backward_time:"+str(profiler.backward_times[node])+", forward_mem:"+str(profiler.forward_mem[node])+str(", backward_mem:")+str(profiler.backward_mem[node]))
    #    for node in profiler.forward_times:
    #        graph._nodes[node.id].forward_times = profiler.forward_times[node]
    #        graph._nodes[node.id].backward_times = profiler.backward_times[node]
    #        graph._nodes[node.id].forward_mem = profiler.forward_mem[node]
    #        graph._nodes[node.id].backward_mem = profiler.backward_mem[node]
    #    graph.serialize("with_time")
        profiler.serialize("profiler_output")
